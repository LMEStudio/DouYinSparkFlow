...
def build_message() -> str:
    today = date.today()
    if config.get("happyNewYear", {}).get("enabled", False) and date(2026, 2, 16) <= today <= date(2026, 3, 3):
        ...
        if "[API]" in message and config.get("hitokotoEnabled", False):
            api_content = request_hitokoto()
            message = message.replace("[API]", api_content)
        if "[date]" in message and config.get("dateEnabled", False):
            message = message.replace("[date]", today.strftime("%Y月%m年%d日"))
